...
// their running bibs.  Combine both the first and last names into a new array called fullName. 
let fullName = [];
...
// Problem 1
// The director ordered the wrong shirts and needs a list of runners that wear small sized shirts so he can order new ones.
// Create an array that lists all runners that wear S sized shirts.
let smallShirts = [];
smallShirts = runners.filter(item => item.shirt_size === "S");
console.log(smallShirts);

// Problem 2
// Revert the power drunken allCaps to all lowercase for aesthetic reasons
let lowerCase = runners.map(item => item.first_name.toLowerCase());
console.log(lowerCase);

// Problem 3
// Another shirt thing but for XL!
let xtraLarge = runners.filter(item => item.shirt_size === "XL");
